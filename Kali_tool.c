//TODO: imageconversion to BMP, report generation, custom steg encoding/decoding
...
//Funct4: LSB
unsigned char *BMP(const char *filename, int p){
...
unsigned char *lsb_analysis(const char *filename, char c){
    ...
    if(strcmp("BMP",detect_file_type(filename,0))==0){pixel=BMP(filename,0);}
    else{
        free(pixel);
        printf("LSB analysis made only for BMP files for now and not %s", detect_file_type(filename,0));
        return NULL;
    };
    ...
    if(LSB==NULL){
        printf("Memory allocation failed for LSB.\n");
        free(pixel);
        return NULL;
    };
...
char *decode_lsb(unsigned char *lsbData){
    ...
    if (bitIndex != 0 && byteIndex == maxBytes) {
        printf("Warning: Decoded message may be incomplete.\n");
    }
...
int encode_lsb(const char *imageFilename, const char *message) {
    ...
    int bitIndex = 0; // Track the bit we're encoding
    for (int i = 0; i < totalBits; i++) {
        int pixelIndex = (bitIndex / 3) * 3 + (bitIndex % 3);

        // Get the current bit to encode (message bit)
        int bit = (message[i / 8] >> (7 - (i % 8))) & 1;

        // Modify the LSB of the pixel byte
        encodedPixelData[pixelIndex] &= ~1;  // Clear the current LSB, ~ is not
        encodedPixelData[pixelIndex] |= bit; // Set the new LSB
    ...
    fseek(file, bmpHeader.offset, SEEK_SET);
    for (int y = height - 1; y >= 0; y--) { // BMP stores pixels from bottom to top
...
void BMP_header(const char *filename, char *out){
    ...
    sprintf(out, "24-bit BMP File Analysis:\nFile Type: BM (Valid BMP)\nFile Size: %u bytes\nCompression Method: ", fileSize);
    char temp[50];

    printf("24-bit BMP File Analysis:\n");
    printf("File Type: BM (Valid BMP)\n");
    ...
    if(compression==0){
        ...
        printf("RLE-8(Run-Length Encoding for 8-bit data)\n");
    }else if(compression==2){
        strcat(out, "RLE-4 (Run-Length Encoding for 4-bit data)\n");
        printf("RLE-4 (Run-Length Encoding for 4-bit data)\n");
    }else{
    ...
    if(colorSpace==0){
        ...
        printf("sRGB(Standard RGB)\n");
    }else{
        sprintf(temp,"Custom Color Profile (0x%X)\n", colorSpace);
        strcat(out, temp);
        printf("Custom Color Profile (0x%X)\n", colorSpace);
    }
...
void report(const char *message){
    ...
    // Add a newline after the message before appending
    char *formatted_message = (char *)malloc(100000 * sizeof(char));
...
int main(int argc, char *argv[]){
    ...
    */
    ...
    /*
    char *message=NULL;
    if(read_message(textfile,&message)!=0){return 1;};
    int encode_result = encode_lsb(inputfile,message,outputfile);

    if (encode_result== 0) {
        printf("Message encoded successfully and saved to %s\n", outputFilename);
    } else {
